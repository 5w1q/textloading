...
async def _get_upstream_json(client: httpx.AsyncClient, url: str) -> dict[str, Any]:
    """请求 Evil0ctal；连接/超时错误转换为可读的 HTTP 异常（避免未捕获导致 500）。"""
    try:
        r = await client.get(url)
    except httpx.TimeoutException as exc:
        raise HTTPException(
            status_code=504,
            detail="上游解析服务响应超时，请稍后重试或检查 Evil0ctal 是否过载。",
        ) from exc
    except httpx.ConnectError as exc:
        raise HTTPException(
            status_code=503,
            detail=(
                "无法连接到 Evil0ctal 上游（compose 默认期望宿主机 :19001）。"
                "请先按仓库 README 在本机启动上游并配置 Cookie；"
                "若上游端口不同，请修改 bridge 服务的 UPSTREAM_BASE_URL 或 UPSTREAM_BASE_URLS。"
                f"（{exc!s}）"
            ),
        ) from exc
    except httpx.HTTPError as exc:
        raise HTTPException(status_code=502, detail=f"上游请求失败: {exc!s}") from exc
    ...
    if not isinstance(payload, dict):
        raise HTTPException(status_code=502, detail="upstream JSON must be an object")
    return payload
...
async def resolve(unique_id: str = Query(..., min_length=1, description="抖音号 unique_id")):
    ...
    """
    uid = unique_id.strip()
    # 用户若粘贴了 sec_uid（MS4wLj…），无需再调上游 get_sec_user_id
    if SECUID_RE.fullmatch(uid):
        return {"sec_uid": uid}
    ...
    async with httpx.AsyncClient(timeout=UPSTREAM_TIMEOUT) as client:
        payload = await _get_upstream_json(client, url)
