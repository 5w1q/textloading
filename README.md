...
- **bridge**（可选）：宿主机 <http://localhost:19000/health>，容器内端口 9000；默认请求本机 **`UPSTREAM_BASE_URL=http://host.docker.internal:19001`** 上的 Evil0ctal 上游，请先在 **19001** 自行启动上游服务。
...
链路顺序：**浏览器 → 本仓库前端/API** → **bridge（容器 19000）** → **Evil0ctal（宿主机 19001，默认）** → 抖音 Web API。缺任何一环，`resolve` / 同步任务会失败或超时。
...
| bridge 健康检查 | <http://localhost:19000/health> | 返回正常 |
| 试解析（把 `抖音号` 换成真实 unique_id） | <http://localhost:19000/douyin/resolve?unique_id=抖音号> | JSON 含 `sec_uid`；若上游未起或 Cookie 无效会 502/错误体 |
...
2. `docker compose up -d --build` 会构建并启动 **[bridge/](bridge/)**（映射宿主机 **19000**）。桥接将上游转为项目所需两个 `GET`（见 [bridge/README.md](bridge/README.md)）。
